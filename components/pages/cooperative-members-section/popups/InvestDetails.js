...
const InvestDetailsPopup = ({ onAction = () => {}, onClose = () => {}, onOpenAddCard = () => {}, activeTopUpType }) => {
  const topUpTypes = ["Wallet", "Bank cards", "Transfer"];
  const [activeCard, setActiveCard] = useState(0);
  const [topUpType, setTopUpType] = useState(activeTopUpType ?? "Wallet");
  return (
    <PopupLayout onClose={onClose} title="Invest Details">
      <TextField name="Enter number of slots" type={"tel"} id="Enter number of slots" label="Enter number of slots" variant="filled" />
...
export default InvestDetailsPopup;
